...
def _collect_head_vectors(layer_idx: int, attn_output: torch.Tensor) -> None:
    ...
    if layer_idx == 0 and not STATE.collected:
        # Lightweight debug to confirm collection is happening
...
def _apply_pc_patching(layer_idx: int, attn_output: torch.Tensor) -> torch.Tensor:
    ...
    # pcs: (k, D), flat: (N, D)
    # proj_coeffs: (N, k) = flat @ pcs^T
    proj_coeffs = flat.to(torch.float32) @ pcs.t()  # (N, k)
    # proj_vecs: (N, D) = sum_k coeff_k * u_k
    proj_vecs = proj_coeffs @ pcs  # (N, D)
    # Strengthen the interference via lambda_scale
...
# --- Patched forward for Qwen2SdpaAttention ---
...
def patched_qwen2_sdpa_forward(
    ...
    This follows the original implementation closely, but adds:
      - _collect_head_vectors on the pre-projection attention output
      - _apply_pc_patching on a single target head before o_proj

    Note: This implementation is intended for analysis in a controlled context
    (Qwen2HeadPatchContext). Outside that context, the original forward is used.
    """
    ...
    # attn_output: (B, num_heads, q_len, head_dim)

    # 1) Collect per-head representations (for PCA) if enabled.
    _collect_head_vectors(self.layer_idx, attn_output)

    # 2) Apply ILM PC patching on a specific head (if enabled).
    attn_output = _apply_pc_patching(self.layer_idx, attn_output)

    # Restore original shapes and apply output projection.
    attn_output = attn_output.transpose(1, 2).contiguous()  # (B, q_len, num_heads, head_dim)
...
class Qwen2HeadPatchContext:
    """
    Context manager that temporarily patches Qwen2SdpaAttention.forward
    to enable ILM head collection / patching.
    """

    def __enter__(self):
        self._backup = Qwen2SdpaAttention.forward
        Qwen2SdpaAttention.forward = patched_qwen2_sdpa_forward
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        Qwen2SdpaAttention.forward = self._backup
        disable_patching()
        reset_collection()
