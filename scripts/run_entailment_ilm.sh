...
# Run original entailment experiments with ILM-based auto layer selection enabled for PiFi.
# Env overrides:
...
for DS in ${DATASETS}; do
  echo "[ILM] Entailment Dataset=${DS} | Model=${MODEL} | LLM=${LLM}"
  ...
  SEL_JSON="./results/layer_selection/entailment/${DS}/${MODEL}/${LLM}/selection.json"
  if [ -f "$SEL_JSON" ]; then
