...
# =============================================================================
# PiFi Entailment Unified Experiment Script
# =============================================================================
# Usage:
#   MODE=base ./run_entailment.sh              # Base model only
#   MODE=pifi ./run_entailment.sh              # PiFi with last layer
#   MODE=pifi_ilm ./run_entailment.sh          # PiFi with ILM auto-selection
#
...
for DS in ${DATASETS}; do
  ...
  python main.py --job preprocessing --task_dataset "$DS" \
    "${COMMON_ARGS[@]}"
  ...
  case $MODE in
    base)
      ...
      python main.py --job training --task_dataset "$DS" --test_dataset "$DS" --method base \
        --num_epochs "$EPOCHS" --batch_size "$BS" --num_workers "$WORKERS" \
        --use_wandb "$USE_WANDB" "${COMMON_ARGS[@]}"

      echo "[3/4] Testing BASE model..."
      python main.py --job testing --task_dataset "$DS" --test_dataset "$DS" --method base \
    ...
    pifi)
      ...
      python main.py --job training --task_dataset "$DS" --test_dataset "$DS" --method pifi --llm "$LLM" \
        --layer_num "$EFFECTIVE_LAYER" \
        --num_epochs "$EPOCHS" --batch_size "$BS" --num_workers "$WORKERS" \
        --use_wandb "$USE_WANDB" "${COMMON_ARGS[@]}"

      echo "[3/4] Testing PiFi model..."
      python main.py --job testing --task_dataset "$DS" --test_dataset "$DS" --method pifi --llm "$LLM" \
        --layer_num "$EFFECTIVE_LAYER" \
        --batch_size "$BS" --num_workers "$WORKERS" \
        --use_wandb "$USE_WANDB" "${COMMON_ARGS[@]}"
    ...
    pifi_ilm)
      ...
      SEL_JSON="$RESULT_PATH/layer_selection/entailment/${DS}/${MODEL}/${LLM}/selection.json"
      if [[ -f "$SEL_JSON" ]]; then
